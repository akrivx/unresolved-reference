...
Call it C with stdlib. C++ began as "C with Classes," and discussion of the language still often centers on features such as classes, inheritance, and templates. Real codebases can look quite different. Much of their C++ may live in the library types they compose rather than in advanced language mechanisms written directly into application code.
...
Modern C++ is broad enough to be intimidating, but its feature set is not a checklist. A codebase can choose a small, coherent core of structs, enums, free functions, direct control flow, and carefully selected library types, and still be modern, scalable C++. Looking like C is not evidence that its authors failed to learn the rest of the language.

Resource ownership is a useful place to see how that small core remains powerful. Most application code should not implement resource ownership, move semantics, or exception-safe cleanup. It should compose types that already implement those things. The hard work still exists, but it is concentrated in a smaller layer of resource-owning primitives.

That leads to two questions. Should engineers adopt this style deliberately, or at least stop treating it as unfinished C++? And should language and library designers make the boundary between ordinary application code and resource-owning primitives easier to work with?
...
None of this introduces a new language mechanism. The model is a name for what RAII and the Rule of Zero imply when applied across a codebase.

**Tier 1** contains resource-owning primitives such as `std::mutex`, `std::ifstream`, and `std::vector`. Their authors deal with ownership, moves, exception safety, and cleanup policy.

**Tier 2** is application code. It combines Tier-1 types into domain behavior. Its special member functions can usually be generated by the compiler, even though the generated destructor may do substantial work through its members.
...
The `Widget` example needs no inheritance, virtual dispatch, operator overloading, or custom templates. That is not an argument against those features when the domain calls for them. Methods that protect invariants, generic algorithms, overloaded operators, and runtime polymorphism may all belong in Tier 2. The tiers describe where ownership and lifecycle machinery live, not the complete design of an application.

The same author-consumer split likely applies beyond resource ownership. Authors of generic facilities may need type traits, `decltype`, `std::declval`, `std::invoke`, forwarding references, or other compile-time machinery, while application code consumes a simpler interface. That broader claim deserves examples of its own; the argument developed here is about ownership and lifecycle.

Nor is Tier 2 a required feature set. It may use ranges and other high-level facilities when they simplify application code, but it need not. A team may reasonably prefer an ordinary loop when it is clearer to maintain, faster to compile, or easier to debug.

If this model is accurate, difficult C++ should cluster at the seams between the tiers. Those are the places where someone must create a new resource-owning primitive or reconcile existing primitives with different contracts.
...
The breadth of modern C++ should be a menu, not a ladder every engineer must climb. Competence is not measured by how many language and library features appear in the code. Choosing a smaller subset after considering maintenance, build time, debuggability, and team familiarity can be an engineering decision rather than a sign of limited knowledge.

## A Question for Language and Library Designers

Should C++ make the boundary between the two tiers easier to work with?

The model does not require a new `tier2` keyword or a ban on custom special member functions. It does suggest a practical test for language and library proposals: does the change make resource-owning primitives harder to write incorrectly, or easier for application code to compose?

Recent standards already provide examples. Guaranteed copy elision in C++17 lets the returned wrapper be constructed directly in the caller. Parenthesized aggregate initialization in C++20 lets that wrapper construct the non-movable `Widget` in place without a user-written constructor. `std::expected` in C++23 gives application boundaries an explicit way to state failure contracts.

None of those features was designed around this two-tier terminology. Each nevertheless removes friction at the boundary. Resource-owning types can retain appropriate constraints, including being non-movable, while Tier-2 authors combine them in aggregates, return types, and error channels without adding special member functions or adapters.

This provides a more concrete question for future work. Does a proposal reduce the chances of making ownership copyable by accident, implementing a broken move, leaking during failed construction, or violating a cleanup guarantee? Does it let application code compose the resulting type without taking on those responsibilities itself?

Enforcement is a harder question. A compiler mode or attribute that rejected custom ownership machinery in an application type might catch mistakes, but it could also reject valid designs. C++ has an enormous existing surface and a large compatibility burden. Static analysis, style guides, and external experiments may be better places to test such restrictions before the standard adopts them.

The two-tier model does not prescribe a particular feature. It offers a way to ask whether features and tools help resource-owning primitives absorb ownership and lifecycle complexity without forcing application code to reproduce it.
...
Feature restraint is not anachronism. The relevant question is whether the chosen abstractions fit the problem and its operational costs, not how much of modern C++ appears in each source file.
