...
That leads to two questions. Should engineers adopt this style deliberately, or at least stop treating it as unfinished C++? And should language and library designers make the boundary between ordinary application code and reusable primitives easier to work with?
...
**Tier 1** contains reusable primitives. In this example, they are resource-owning types such as `std::mutex`, `std::ifstream`, and `std::vector`. Their authors deal with ownership, moves, exception safety, and cleanup policy.

**Tier 2** is application code. It combines Tier-1 types and facilities into domain behavior. Its special member functions can usually be generated by the compiler, even though the generated destructor may do substantial work through its members.
...
The `Widget` example needs no inheritance, virtual dispatch, operator overloading, or custom templates. That is not an argument against those features when the domain calls for them. Methods that protect invariants, generic algorithms, overloaded operators, and runtime polymorphism may all belong in Tier 2. The tiers describe where ownership, lifecycle, and generic-programming machinery live, not the complete design of an application.

The same author-consumer split applies beyond resource ownership. Authors of reusable facilities may need type traits, `decltype`, `std::declval`, `std::invoke`, forwarding references, or other compile-time machinery. Application code can consume the resulting interface without reproducing that machinery. Ranges and other high-level facilities can therefore belong in Tier 2 when they simplify the application, but they are optional: a team may reasonably prefer an ordinary loop when it is clearer to maintain, faster to compile, or easier to debug.

If this model is accurate, difficult C++ should cluster at the seams between the tiers. Those are the places where someone must create a new primitive or reconcile existing primitives with different contracts.
...
The model does not require a new `tier2` keyword or a ban on custom special member functions. It does suggest a practical test for language and library proposals: does the change make reusable primitives harder to write incorrectly, or easier for application code to compose?
...
The two-tier model does not prescribe a particular feature. It offers a way to ask whether features and tools help reusable primitives absorb ownership, lifecycle, and generic-programming complexity without forcing application code to reproduce it.
